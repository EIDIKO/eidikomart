...
describe('getJsonData', () => {
  it('Successful GET request', async () => {
    ...
    // Assertions
    expect(result).toEqual(mockResponse); // Match entire object

    // Alternatively, you can assert specific properties
  ...
  it('Failure when the HTTP request is not successful', async () => {
    const mockErrorResponse = {
      status: '404',
      message: 'HTTP error! Status: 404',
    };
  
    // Mock the fetch function to simulate an unsuccessful response
    global.fetch = jest.fn().mockResolvedValueOnce({
      ok: false,
      status: 404,
    ...
    const apiUrl = 'https://dummy.restapiexample.com/api/v1/employeesa';
  
    // Assertions for failure case
    await expect(getJsonData(apiUrl)).rejects.toThrowError(
